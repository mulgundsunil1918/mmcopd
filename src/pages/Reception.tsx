...
import { Search, UserPlus, Phone, Cake, Pencil, Calendar as CalendarIcon, IdCard, ChevronDown, ChevronRight, FileText, Loader2 } from 'lucide-react';
...
/**
 * One past visit. Collapsed it shows date/doctor/status; expanded it fetches that
 * visit's consultation (the typed OPD slip) and shows History / Examination /
 * Impression / Advice / follow-up, plus any custom template fields. Fetch is lazy
 * (only when opened) so the list stays light.
 */
function PastVisitRow({ r }: { r: any }) {
  const [open, setOpen] = useState(false);
  const { data: consult, isLoading } = useQuery({
    queryKey: ['past-consult', r.id],
    queryFn: () => window.electronAPI.consultations.getByAppointment(r.id),
    enabled: open,
  });
  const extras = (consult?.extra_fields && typeof consult.extra_fields === 'object') ? consult.extra_fields : {};
  const hasNotes = !!consult && !!(consult.history || consult.examination || consult.impression || consult.advice || Object.keys(extras).length);

  return (
    <li className="py-2.5">
      <div className="flex items-start justify-between gap-2">
        <button className="flex items-start gap-2 flex-1 min-w-0 text-left" onClick={() => setOpen((o) => !o)}>
          {open ? <ChevronDown className="w-4 h-4 mt-0.5 text-gray-400 shrink-0" /> : <ChevronRight className="w-4 h-4 mt-0.5 text-gray-400 shrink-0" />}
          <div className="min-w-0">
            ...
            {r.notes && <div className="text-xs text-gray-600 dark:text-slate-300 mt-1 italic truncate">“{r.notes}”</div>}
          </div>
        </button>
      ...
      {open && (
        <div className="mt-2 ml-6 rounded-lg border border-gray-200 dark:border-slate-700 bg-gray-50/60 dark:bg-slate-800/40 p-3 text-xs space-y-2">
          {isLoading ? (
            <div className="flex items-center gap-2 text-gray-400"><Loader2 className="w-3.5 h-3.5 animate-spin" /> Loading notes…</div>
          ) : !hasNotes ? (
            <div className="flex items-center gap-1.5 text-gray-400 italic"><FileText className="w-3.5 h-3.5" /> No typed OPD slip / notes were recorded for this visit.</div>
          ) : (
            <>
              <ConsultField label="History / Complaints" value={consult!.history} />
              <ConsultField label="Examination" value={consult!.examination} />
              <ConsultField label="Impression" value={consult!.impression} />
              <ConsultField label="Advice / Plan" value={consult!.advice} />
              {Object.entries(extras).map(([k, v]) => <ConsultField key={k} label={k.replace(/_/g, ' ')} value={v as string} />)}
              {consult!.follow_up_date && (
                <div className="text-[11px] text-emerald-700 dark:text-emerald-300">Follow-up: {fmtDate(consult!.follow_up_date)}</div>
              )}
            </>
          )}
        </div>
...
function ConsultField({ label, value }: { label: string; value?: string | null }) {
  if (!value || !String(value).trim()) return null;
  return (
    <div>
      <div className="text-[10px] uppercase tracking-wide font-semibold text-gray-500 dark:text-slate-400 capitalize">{label}</div>
      <div className="text-[12px] text-gray-800 dark:text-slate-200 whitespace-pre-wrap">{value}</div>
    </div>
  );
}
